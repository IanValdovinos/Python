...
class Graph:
    ...
    def explore(self):
        ...
        while not current_room == "treasure room":
            node = self.graph_dict[current_room]

            for connected_room, weight in node.edges.items():
                key = connected_room[0]
                print("enter {} for {}: {} cost".format(key, connected_room,
                                                        weight))

            valid_choices = [room_name[0] for room_name in node.edges.keys()]
            print("\nYou have accumulated: {} cost".format(path_total))

            choice = input("\nWhich room do you move to?")

            if choice not in valid_choices:
                print("please select from these letters: {}".format(
                    valid_choices))
            else:
